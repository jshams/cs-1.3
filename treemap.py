...
class BinaryTreeMap(object):
    ...
    def insert(self, key, val):
        ...
        if key < parent.data and parent.left == None: # Check if the given item should be inserted left of parent node
            parent.left = BinaryTreeNode((key, val))  # Create a new node and set the parent's left child
            self.size += 1 # Increase the tree size
        elif key > parent.data and parent.right == None: # Check if the given item should be inserted right of parent node
            parent.right = BinaryTreeNode((key, val)) # Create a new node and set the parent's right child 
            self.size += 1 # Increase the tree size
    ...
    def _find_parent_node_iterative(self, key):
        ...
        parent = None 
        while node is not None: # Loop until we descend past the closest leaf node
            if node.data[0] == key: # Check if the given item matches the node's data
                return parent # Return the parent of the found node
            elif key < node.data[0]: # Check if the given item is less than the node's data
                parent = node # Update the parent
                node = node.left # descend to the node's left child
            elif key > node.data[0]:  # Check if the given item is greater than the node's data
                parent = node # Update the parent
                node = node.right # descend to the node's right child
...
def test_binary_search_tree():
    ...
    # items = [2, 1, 3]
    items = [4, 2, 6, 1, 3, 5, 7]
    items = [41, 20, 65, 11, 29, 32, 50, 91, 72, 99]
